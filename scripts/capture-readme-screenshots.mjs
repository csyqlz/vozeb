...
const puppeteerEntry =
  process.env.PUPPETEER_CORE_ENTRY ||
  path.join(
    process.env.TEMP || ".",
    "vozeb-screenshot-tools",
    "node_modules",
    "puppeteer-core",
    "lib",
    "esm",
...
const baseUrl =
  process.env.VOZEB_SCREENSHOT_BASE_URL || "http://localhost:3000";
...
<svg xmlns="http://www.w3.org/2000/svg" width="900" height="640" viewBox="0 0 900 640">
  ...
  <text x="70" y="115" fill="#f8fafc" font-family="Arial, sans-serif" font-size="56" font-weight="700">VOZEB</text>
  ...
  nodes: [
    {
      ...
      metadata: {
        content:
          "用无限画布整理参考图、提示词、模型配置和生成结果，保留每一次可复用的创作路径。",
        prompt: "创作工作流说明",
        status: "success",
        fontSize: 16,
      },
    ...
    },
    {
      ...
      metadata: {
        content: imageOne,
        prompt: "VOZEB AI canvas concept frame",
        status: "success",
        naturalWidth: 900,
        naturalHeight: 640,
      ...
      messages: [
        {
          id: "m1",
          role: "user",
          text: "帮我把这组节点整理成可复用的海报创作流程。",
        },
        {
          id: "m2",
          role: "assistant",
          text: "已识别提示词、配置节点和两张参考图，可继续生成或保存到素材库。",
        },
      ],
    },
  ],
...
const demoAssets = [
  ...
  },
  {
    ...
    updatedAt: new Date().toISOString(),
    data: {
      content: "为 AI 创作工具生成一个清晰、有层次、突出工作流的产品主视觉。",
    },
  },
];
...
async function main() {
  ...
  const { default: puppeteer } = await import(
    pathToFileURL(puppeteerEntry).href
  );
  const userDataDir = path.join(
    process.env.TEMP || ".",
    `vozeb-readme-chrome-${Date.now()}`,
  );
  ...
  try {
    ...
    await capture(page, "/image", "04-image-workbench.png", async () => {
      await typeIntoFirstTextarea(
        page,
        "一张 VOZEB AI 创作工作台宣传图，展示无限画布、提示词库和生成结果。",
      );
    });
    await capture(page, "/video", "05-video-workbench.png", async () => {
      await typeIntoFirstTextarea(
        page,
        "镜头沿画布连线推进，展示从参考图到成片的 AI 视频创作流程。",
      );
    });
  ...
  await page.evaluate(
    async ({ project, assets }) => {
      ...
      await putIdb(
        "app_state",
        "vozeb:canvas_store",
        JSON.stringify({ state: { projects: [project] }, version: 0 }),
      );
      await putIdb(
        "app_state",
        "vozeb:asset_store",
        JSON.stringify({ state: { assets }, version: 0 }),
      );
    ...
    async ({ coverUrl }) => {
      await fetch("/api/my-prompts", {
        ...
        body: JSON.stringify({
          title: "VOZEB 产品主视觉",
          prompt:
            "生成一张干净、专业、适合开源项目 README 展示的 AI 创作工作台界面图。",
          category: "产品展示",
          tags: ["README", "工作流", "主视觉"],
          coverUrl,
        }),
...
async function maskSensitive(page) {
  await page.evaluate(() => {
    const sensitivePattern =
      /(api|key|token|secret|password|密钥|授权码|密码|邮箱|email|base url|baseurl)/i;
    ...
    document.querySelectorAll("[title]").forEach((el) => {
      ...
      if (sensitivePattern.test(title) || title.includes("@"))
        el.setAttribute("title", "已隐藏");
    });
  });
}
