...
插件源码位于仓库的 `plugins/vozeb-canvas/` 目录。
...
确保 `~/.agents/plugins/marketplace.json` 中有 VOZEB Canvas 条目。这里的 `path` 要指向仓库里的插件子目录：
...
仓库内的 `.agents/plugins/marketplace.json` 已经指向 `./plugins/vozeb-canvas`，适合开发者本地调试。
...
- `plugins/vozeb-canvas/.codex-plugin/plugin.json`
- `plugins/vozeb-canvas/.mcp.json`
- `plugins/vozeb-canvas/skills/` 下的技能说明
- 插件暴露的 MCP、app 或入口脚本
