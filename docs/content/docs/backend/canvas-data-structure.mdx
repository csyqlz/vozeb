...
- 画布项目 JSON：`localForage`，数据库名 `vozeb`，storeName `app_state`，key 为 `vozeb:canvas_store`。
- 我的素材 JSON：`localForage`，数据库名 `vozeb`，storeName `app_state`，key 为 `vozeb:asset_store`。
- 图片 Blob：单独存到 `localForage` 实例，数据库名 `vozeb`，storeName `image_files`。
- 视频等媒体 Blob：单独存到 `localForage` 实例，数据库名 `vozeb`，storeName `media_files`。
