...
export function loadConfig(create = false): CanvasAgentConfig {
    try {
        ...
        if (legacyConfig) {
            if (create) saveConfig(legacyConfig);
            return legacyConfig;
        }
...
function readLegacyConfig() {
    try {
        return JSON.parse(fs.readFileSync(LEGACY_CONFIG_FILE, "utf8")) as CanvasAgentConfig;
    } catch {
        return null;
    }
}
