...
- 后台系统设置新增“邮箱注册”开关，开启后注册页必须填写唯一邮箱。
- 后台新增 SMTP 邮箱服务配置，默认 QQ 邮箱 `smtp.qq.com:465 SSL`，也支持网易、企业邮箱和其他自定义 SMTP。
- 邮箱服务区域新增“测试邮箱”按钮，可用当前表单配置直接发送测试邮件，便于上线前确认授权码、端口和 SSL 是否正确。
- 前端注册表单同步支持邮箱字段和 6 位邮箱验证码，只有管理员开启邮箱注册时才显示；未验证邮箱不能直接注册登录。
- 登录页新增“忘记密码”，可通过绑定邮箱验证码重置密码。
- 账号菜单新增“个人资料”，普通用户和管理员都可修改昵称、绑定邮箱和登录密码。
- 管理员用户管理支持修改用户昵称、绑定邮箱、重置密码，并可删除用户。
- 首页底部新增版权所有、使用条款、隐私政策和社交媒体入口，后台可配置邮箱联系、Telegram、X、Instagram 的开关和链接。
- 内置默认 `/terms` 使用条款页和 `/privacy` 隐私政策页，部署后可直接使用或在后台改成外部链接。
- 管理员公共提示词新增表单重新排版，标题、分类、标签、封面 URL、正文和备注更清楚。
- 首页提示词展示区改成上浮承接面板，替换之前突兀的中段线条，浅色/深色模式都更自然。
- 工作台输入光标问题修复，空提示词时不再被透明覆盖层挡住光标。
- Next.js 源码构建默认限制为 1 个 worker；0.5G 服务器仍建议只拉取发布镜像，避免现场构建拉爆 CPU。
- 版本提升为 `v0.6.0`，README、CHANGELOG 和检查更新版本同步。
...
- 文档补充低配服务器构建拉满 CPU 的原因：当前版本包含 Next.js 生产构建、Turbopack 页面生成、账号后台、提示词种子和 standalone 镜像步骤，和原作者早期纯前端/轻量构建阶段不同；0.5G/低配服务器建议只拉取发布镜像，确需源码构建时放到本机或 CI 执行。
